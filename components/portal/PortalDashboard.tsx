    ...
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        >
          ...
          <div>
            <span className="text-[10px] font-sans font-bold text-[var(--color-brand-muted)] uppercase tracking-wider block">Project Velocity</span>
            <h4 className="font-serif font-bold text-lg text-[var(--color-brand-dark)]">
              {clientProjects.length > 0 ? `${clientProjects[0].progress}% Complete` : "No Active Project"}
            </h4>
          </div>
        </motion.div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        {/* LEFT COLUMN: Meetings, Projects, Payments (2/3 width) */}
        <div className="lg:col-span-2 space-y-8">
          ...
          <div className="bg-white border border-[var(--color-brand-neutral)]/20 rounded-2xl p-6 shadow-2xs space-y-4">
            <div className="flex justify-between items-center border-b border-[var(--color-brand-neutral)]/15 pb-3">
              <h4 className="font-serif font-bold text-sm text-[var(--color-brand-primary)] uppercase tracking-wider flex items-center space-x-2">
                <Calendar className="w-4.5 h-4.5 text-[var(--color-brand-primary)]" />
          ...
          <div className="bg-white border border-[var(--color-brand-neutral)]/20 rounded-2xl p-6 shadow-2xs space-y-4">
            <div className="flex justify-between items-center border-b border-[var(--color-brand-neutral)]/15 pb-3">
              <h4 className="font-serif font-bold text-sm text-[var(--color-brand-dark)] uppercase tracking-wider flex items-center space-x-2">
                <Briefcase className="w-4.5 h-4.5 text-[var(--color-brand-primary)]" />
                <span>Active Deliverable Track</span>
              </h4>
              <span className="text-[9px] font-sans font-bold text-[var(--color-brand-muted)] uppercase tracking-wider">
                Milestones
              </span>
            ...
            {clientProjects.length > 0 ? (
                ...
                {clientProjects.map((p) => (
                  <div key={p.id} className="space-y-3">
                    <div className="flex justify-between items-start gap-3">
                      <div>
                        <h5 className="font-serif font-bold text-base text-[var(--color-brand-dark)]">{p.title}</h5>
                        <p className="text-xs text-[var(--color-brand-muted)] mt-1 leading-relaxed">{p.description}</p>
                      </div>
                      <span className="bg-[var(--color-brand-panel)] text-[var(--color-brand-primary)] border border-[var(--color-brand-neutral)]/25 text-[8px] font-sans font-bold uppercase px-2.5 py-0.5 rounded shrink-0">
                        {p.status.replace("_", " ")}
                      </span>
                    </div>
                    ...
                    <div className="space-y-1">
                      <div className="flex justify-between text-[10px] font-mono text-[var(--color-brand-muted)]">
                        ...
                        <span className="font-bold text-[var(--color-brand-primary)]">{p.progress}%</span>
                      </div>
                      <div className="h-2 bg-[var(--color-brand-bg)] border border-[var(--color-brand-neutral)]/25 rounded-full overflow-hidden">
                        <motion.div 
                          initial={{ width: 0 }}
                    ...
                    <div className="grid grid-cols-2 gap-4 text-[10px] font-mono text-[var(--color-brand-muted)] pt-1">
                      <div>
                        <span className="block text-[8px] font-sans font-bold uppercase tracking-wider text-[var(--color-brand-muted)]/80">Commenced</span>
                        <span className="font-bold text-[var(--color-brand-dark)]">{p.startDate}</span>
                      </div>
                      <div>
                        <span className="block text-[8px] font-sans font-bold uppercase tracking-wider text-[var(--color-brand-muted)]/80">Target Launch</span>
                        <span className="font-bold text-[var(--color-brand-primary)]">{p.targetLaunch}</span>
                      </div>
                    </div>
                  </div>
                ))}
              ...
              <div className="p-8 text-center bg-[var(--color-brand-bg)] border border-dashed border-[var(--color-brand-neutral)]/20 rounded-xl">
                <Briefcase className="w-6 h-6 text-[var(--color-brand-muted)] mx-auto mb-2 opacity-60" />
                <p className="text-xs text-[var(--color-brand-muted)] font-mono">No active engineering project scopes found.</p>
              </div>
            )}
          </div>
        </div>

        {/* RIGHT COLUMN: Alerts and chat */}
        <div className="space-y-8">
          {/* Alerts */}
          <div className="bg-white border border-[var(--color-brand-neutral)]/20 rounded-2xl p-6 shadow-2xs space-y-4">
            <div className="flex justify-between items-center border-b border-[var(--color-brand-neutral)]/15 pb-3">
                  ...
                  >
                    {!n.read && (
                      <button 
                        ...
                        className="absolute top-2 right-2 text-[9px] font-sans font-bold text-[var(--color-brand-primary)] hover:text-[var(--color-brand-dark)] uppercase tracking-wider"
                      >
                        Dismiss
                      </button>
                    )}
          ...
          <div className="bg-white border border-[var(--color-brand-neutral)]/20 rounded-2xl p-6 shadow-2xs space-y-4">
            ...
            <button
              ...
              className="w-full py-3 bg-[var(--color-brand-primary)] hover:bg-[var(--color-brand-dark)] text-white text-xs font-bold tracking-widest uppercase rounded-xl flex items-center justify-center space-x-2 cursor-pointer transition-colors"
            >
